library(plyr)
setwd("C:\\PERSO\\COURSERA\\Data Cleaning\\WEEK 4")
# Step 1
# Merge the training and test sets to create one data set
###############################################################################

#import feature data + col name
features     = read.table('features.txt',header=FALSE);
#import activity_labels data + col name
...
#import y_train data + col name
y_train <- read.table("train/y_train.txt",header=FALSE)
colnames(y_train)        = "ActivityId";
#import subject_train + col name
...
#import x_test data + col name
x_test <- read.table("test/X_test.txt")
colnames(x_test)        = features[,2];
#import y_test data + col name
y_test <- read.table("test/y_test.txt")
colnames(y_test)        = "ActivityId";
#import subject_test + col name
...
ColNames  = colnames(FinalData3); 
  ...
  # replace t at the beginning by Time
  ColNames = gsub("^(t)","Time",ColNames)
  # replace f at the beginning by Freq
  ...
  # replace GyroJerk by AngularAcceleration
  ColNames <- gsub('GyroJerk',"AngularAcceleration",ColNames)
  # replace Gyro by AngularSpeed
